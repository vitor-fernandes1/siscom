...
<div class="box box-primary">
  ...
  <table id="table_id" class="display">
   ...
   <tbody>
      @foreach($obterManutencaoNaoConcluida as $item)
      <tr>
         <td>{{ $item->pk_manutencao }}</td>
         <td>{{ $item->ds_descricao_manutencao }}</td>
         <td>{{ $item->vl_valor_manutencao }}</td>
         <td>{{ $item->dt_manutencao }}</td>
         @if( $item->fk_pk_situacao === 1)
          <td>Em Andamento</td>
         @else if($item->fk_pk_situacao  === 2)
          <td>Pendente</td>
         @endif
         <td>
            <a href="baixar-manutencao/{{ $item->pk_manutencao }}"><button class="btn btn"><i class="far fa-hand-pointer text-primary"></i> Selecionar</button></a>
         </td>
      </tr>
