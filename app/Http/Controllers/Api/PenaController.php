...
class PenaController extends Controller
{
    ...
    */
    ...
    protected $relationships = ['servicos', 'pecunias', 'multas', 'custas']; 
    ...
    */
    ...
    public function __construct(Pena $model)
    {
        $this->model = $model;
    }
    ...
    */
    public function store(Request $request)
    {
       ...
       //obtem o id da vara
       $varaId = $request->vara;
       $ativo = (Object) $this->model->regraAtivo($varaId);
    ...
    */
    ...
    public function destroy($id)
    {
        ...
        }else
        {
            ...
            $data = [
                'ds_motivo'     => "Exclusão da pena {$id} realizada.",
                'ds_acao_motivo'=> $action[1],
                'ds_tipo_motivo'=> 'App\Models\Pena',
            ];
    ...
    public function ativarOuInativar(Request $request, $id)
    {
        ...
        if($pena != null)
        {
           ...
           //verifica a ação
            if($action == "ativar")
            {
                $ativar = $this->model->find($id)->update(['ds_ativo_pena' => 1]);
            }
            else
            {
                $inativar = $this->model->find($id)->update(['ds_ativo_pena' => 0]);
            }
            //verificar se o motivo foi preenchido
            if(! $request->filled('descricao') || ! $request->has('descricao'))
            {
                $error['message'] = "Para ativar ou desativar uma pena, um motivo deve ser informado !";
                $error['error']   = true;
