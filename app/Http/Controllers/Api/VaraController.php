...
class VaraController extends Controller
{
    ...
    */
    ...
    {
        //obter o numero de identificação da secsubsec
        $obterCdSecsubsec = $request->search ;
        $obterCdSecsubsec = explode('=', $obterCdSecsubsec);
        $cdSecsubsec = $obterCdSecsubsec['1'] ;

        //obter numero de identificação da vara
        $cdVara = $request->where['cd_vara'] ;

        //gerando a cache key
        $cacheKey = "varas-$cdSecsubsec-$cdVara" ;

        //implementando cache
        if(!cache::has($cacheKey))
        {
            $obtemConteudo = $this->indexTrait($request);
            cache::put($cacheKey, $obtemConteudo, 3 );
            return $obtemConteudo;
        }
        
        $obterVaraEmCache = cache::get($cacheKey);
        return $obterVaraEmCache;
    }
    ...
    */
    ...
    public function destroy($id)
    {
        ...
        }else
        {
            ...
            $data = [
                'ds_motivo'     => "Exclusão da vara {$id} realizada.",
                'ds_acao_motivo'=> $action[1],
                'ds_tipo_motivo'=> 'App\Models\Vara',
            ];
    ...
    public function ativarOuInativar(Request $request, $id)
    {
        ...
        if($vara != null)
        {
           ...
           //verifica a ação
            if($action == "ativar")
            {
                $ativar = $this->model->find($id)->update(['ds_ativo_vara' => 1]);
            }
            else
            {
                $inativar = $this->model->find($id)->update(['ds_ativo_vara' => 0]);
            }
            //verificar se o motivo foi preenchido
            if(! $request->filled('descricao') || ! $request->has('descricao'))
            {
                $error['message'] = "Para ativar ou desativar uma vara, um motivo deve ser informado !";
                $error['error']   = true;
