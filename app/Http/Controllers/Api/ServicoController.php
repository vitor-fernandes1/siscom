...
class ServicoController extends Controller
{
    ...
    */
    ...
    protected $relationships = ['pena','entidade','frequencias','tipoServico']; 
    ...
    */
    public function store(Request $request)
    {
       ...
       $ativo = (Object) $this->model->regraAtivo($idPena,  $idEntidade);
     
       if(isset($ativo->error))
       {
           return  $this->createResponse($ativo, 422);
       }
        //adiciona o input minutosPrestados Caso não exista
       if(! $request->has('minutosPrestados')) 
       {
         $request->request->add(['minutosPrestados' => 0]);
         
       }
       //adiciona o input minutosSensibilizados Caso não exista
       if(! $request->has('minutosSensibilizados')) 
       {
         $request->request->add(['minutosSensibilizados' => 0]);
       }

        $data = ['minutosPrestados' => $request->minutosPrestados];
         //chamar e validar regras de negocio
        $status = (Object) $this->model->regraMinutos($data);
     
        //atribui o valor convertido para o input antes de gravar
        $request->merge(['minutosPrestados' => $status->minutosPrestados]);
    
        $servico = $this->storeTrait($request);
        //Guardar as informações no historico 
        $responseServico = $servico->original['Resposta']['conteudo'];

        if(! isset($responseServico->error) )
        {
            //obtem o id da pecunia
            $id = $responseServico->id;
  
            $arrayServico = [
                'nr_min_sensibilizados_novo'     => $request->minutosSensibilizados,
                'nr_hrs_novo'                    => $request->horasTotais,
                'nr_min_prestados_novo'          => $request->minutosPrestados,
                'nr_min_hrs_novo'                => $request->horasMin,
                'nr_max_hrs_novo'                => $request->horasMax,
                'nr_mes_minimo_novo'             => $request->prazoMinCumprimento,
                'fk_pk_tipo_servico_novo'        => $request->tipoHoras,
                'fk_pk_entidade_novo'            => $request->entidade,
                'ds_id_origem'                   => $responseServico->id,
                'ds_tipo_origem'                 => 'App\Models\Servico'
            ];
            //busca a pecunia que foi inserida e insere o historico atraves do relacionamento
            $historicoServico = $this->model->find($id);  
            $historicoServico->historicoServicos()->create($arrayServico);
    ...
    */
    public function update(Request $request, $id)
    {
        ...
        // Pega os valores anteriores antes de fazer o update
        $servicoAnterior = (Object) $this->model->find($id);
        $servicoAnterior = (Object) $this->columnsShow($servicoAnterior);
         
       //adiciona o input minutosPrestados Caso não exista
       if(! $request->has('minutosPrestados')) 
       {
            $request->request->add(['minutosPrestados' => $servicoAnterior->minutosPrestados]);
         
       }
       //adiciona o input minutosSensibilizados Caso não exista
       if(! $request->has('minutosSensibilizados')) 
       {
            $request->request->add(['minutosSensibilizados' => $servicoAnterior->minutosSensibilizados]);
       }
        ...
        $dadosAnteriores = $this->model->regraHistorico($id, null, $obterDadosAnteriores = 1, null);
        $dadosAnteriores = (Object) $dadosAnteriores;
        
        $servico = $this->updateTrait($request, $id);
        //Guardar as informações no historico 
        $responseServico = $servico->original['Resposta']['conteudo'];
        ...
        if(! isset($responseServico->error) )
        {
            ...
            $dadosAtualizados = $this->model->regraHistorico($id, $dadosAnteriores, $atualizarDados = 2, $responseServico->id);
    ...
    public function destroy($id)
    {
          ...
          }else
          {
            ...
            $data = [
                'ds_motivo'     => "Exclusão do servico {$id} realizada.",
                'ds_acao_motivo'=> $action[1],
                'ds_tipo_motivo'=> 'App\Models\Servico',
            ];
            //grava o motivo automatico do motivo da exclusao
            $motivo = $this->model->find($id)->motivos()->create($data);
            //deletar o historico
            $historicoDelete = $this->model->find($id)->historicoServicos()->delete();
    ...
    public function ativarOuInativar(Request $request, $id)
    {
        ...
        if($servico != null)
        {
           ...
           //verifica a ação
            if($action == "ativar")
            {
                $ativar = $this->model->find($id)->update(['ds_ativo_servico' => 1]);
            }
            else
            {
                $inativar = $this->model->find($id)->update(['ds_ativo_servico' => 0]);
            }
            //verificar se o motivo foi preenchido
            if(! $request->filled('descricao') || ! $request->has('descricao'))
            {
                $error['message'] = "Para ativar ou desativar um servico, um motivo deve ser informado !";
                $error['error']   = true;
