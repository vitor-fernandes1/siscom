...
class EnderecoController extends Controller
{
    ...
    */
    ...
    /**
     * <b>getEndereco:</b> Método responsável em realizar um requisição via GET para o VIACEP e retornar com 
     * os dados do endereço informando como parametro o CEP (caso o mesmo esteja correto)
     */
    ...
    public function getEndereco($cep)
    {
        ...
        if($limit !=8)
        {
            return 'O CEP informado não contem a quantidade de números esperado';
        ...
        }else
        {
            ...
            $request = $client->get("{$cep}/json", ['http_errors' =>false]);

            $statusCode = $request->getStatusCode();

            if($statusCode !=200)
            {
               return  "A requisição não foi bem sucedida status code: {$statusCode}";
       
            }else
            {
                $response = $request->getBody();
                return $this->createResponse(json_decode($response, true));
            }
